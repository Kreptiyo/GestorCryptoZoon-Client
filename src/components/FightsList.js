...
export default class FightsList extends Component {
    ...
    getFights = async () => {
        ...
        {
            const res = await axios.get('http://localhost:4000/api/fights/byzoan/' + this.props.match.params.zoan_id);
            this.setState({ fights: res.data });
        }
        else
        {
            const res = await axios.get('http://localhost:4000/api/fights');
            this.setState({ fights: res.data });
        }
    }
    ...
    deleteFight = async (fightId) => {
        if (window.confirm('Are you sure you want to delete it?')) {
            await axios.delete('http://localhost:4000/api/fights/' + fightId);
            this.getFights();
        }
