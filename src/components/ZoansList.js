...
export default class ZoansList extends Component {
    ...
    getZoans = async () => {
        const res = await axios.get('http://localhost:4000/api/zoans')
        this.setState({
            zoans: res.data
        });
    }
    ...
    deleteZoan = async (zoanId) => {
        if (window.confirm('Are you sure you want to delete it?')) {
            await axios.delete('http://localhost:4000/api/zoans/' + zoanId);
            this.getZoans();
        }
    }
    ...
    resetZoonEarned = async (zoanId) => {
        if(window.confirm('Are you sure you want to reset the funds of this zoan?')){
            await axios.put('http://localhost:4000/api/zoans/resetzoonearned/' + zoanId);
            this.getZoans();
        }
    }
